...
const AsideUsers = () => {
  ...
  return (
    <div className='flex-col hidden p-2 pt-2 pl-0 my-0 ml-0 mr-0 rounded-none sm:flex top-2 bottom-2 w-80 bg-slate-100 '>
      <div className='overflow-y-scroll users min-w-max'>
        <h2 className='text-xl  font-extrabold  p-2 bg-gray-200  text-slate-800  z-[100] sticky top-0 rounded-r-full'>
          Messages
        </h2>
        {isLoadingMsg ? (
                    ...
                    {user?._id.toString() != connectedUser?._id.toString() ? (
                      ...
                      >
                        ...
                        <div className='w-full'>
                          <h3 className='text-gray-800 cursor-pointer text-md'>
                            <span>{user.name}</span>{' '}
                            <span>{user.lastname}</span>
                          </h3>
                          <div className='flex justify-between w-full text-xs text-gray-800'>
        ...
        <div className='w-full'>
          {isLoading ? (
            <div className='flex flex-col flex-wrap items-end justify-end w-full p-2 text-gray-800'>
              <h2 className='text-xl  font-extrabold  p-2 bg-gray-800  text-blue-400 sticky top-0  z-[100] rounded-r-full'>
                Contacts
              </h2>
              <User />
              <User />
            ...
            <>
              {users ? (
                <div className=''>
                  <h2 className='text-xl ml-0 p-2 font-extrabold text-slate-800 pt-2 sticky top-0 bg-gray-200 rounded-none z-[100] rounded-r-full'>
                    Contacts
                  </h2>
                  {users.map((user: any) => (
                    <div key={user._id}>
                      {user?._id.toString() != connectedUser?._id.toString() ? (
                        ...
                        >
                          ...
                          <div>
                            <h3 className='cursor-pointer text-slate-800 text-md'>
                              <span>{user.name}</span>{' '}
                              <span>{user.lastname}</span>
                            </h3>
